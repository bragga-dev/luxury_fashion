...
from luxury_fashion.apps.website.schemas.contact_schema import ContactCreateIn, ContactOut
...
def create_contact_message(data: ContactCreateIn) -> ContactOut:
    """
    Cria uma mensagem de contato enviada pelo formulário público.
    Nasce sempre com status PENDING — `status` nunca é aceito do payload.
    """
...
def get_contact_for_admin(contact_id: uuid.UUID) -> ContactOut:
    contact = _get_contact_or_raise(contact_id)
    return ContactOut.from_orm(contact)


def list_contacts_queryset(status: Optional[str] = None, search: Optional[str] = None):
    """QuerySet bruto de contatos, para paginação na camada de router."""
    return get_all_contacts(status=status, search=search)


def update_contact_status_for_admin(contact_id: uuid.UUID, status: str) -> ContactOut:
    contact = _get_contact_or_raise(contact_id)
    contact = update_contact_status(contact, status=status)
    return ContactOut.from_orm(contact)
